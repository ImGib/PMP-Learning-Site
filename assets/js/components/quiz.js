...
export function initQuiz() {
  ...
  let answered = 0, correct = 0, total = 0, cat = null;
  const scoreEl = qs('#quizScore'), progEl = qs('#quizProg'), totalEl = qs('#quizTotal'),
        doneEl = qs('#quizDone'), filterEl = qs('#quizFilter');
  ...
  function update() {
    ...
    if (answered === total && total > 0) {
      ...
      const msg = pct >= 80 ? 'Xuất sắc! Sẵn sàng cho phần này.'
        : pct >= 65 ? 'Đạt ngưỡng — ôn thêm các câu sai.'
      ...
      if (rb) rb.addEventListener('click', () => {
        render(cat);
        const s = qs('#quiz'); if (s) s.scrollIntoView({ behavior: 'smooth', block: 'start' });
      });
    } else {
  ...
  function render(c) {
    cat = c || null; answered = 0; correct = 0;
    ...
    items.forEach((item, qi) => {
      ...
      const opts = item.opts
        .map((o, oi) => `<div class="quiz__opt" data-oi="${oi}"><span class="quiz__key">${keys[oi]}</span>${esc(o)}</div>`)
        .join('');
      q.innerHTML = `
      ...
      q.querySelectorAll('.quiz__opt').forEach((opt) => {
        opt.addEventListener('click', () => {
          ...
          const oi = parseInt(opt.dataset.oi, 10), cor = item.correct;
          q.querySelectorAll('.quiz__opt').forEach((o) => {
            if (parseInt(o.dataset.oi, 10) === cor) o.classList.add('quiz__opt--correct');
          });
          if (oi !== cor) opt.classList.add('quiz__opt--wrong'); else correct++;
          answered++; update();
        });
      });
    });

    if (filterEl) filterEl.querySelectorAll('button').forEach((b) =>
  ...
  if (filterEl) {
    const order = [], cnt = {};
    QUIZ.forEach((it) => { if (cnt[it.cat] === undefined) { cnt[it.cat] = 0; order.push(it.cat); } cnt[it.cat]++; });
    ...
    order.forEach((c) => { html += `<button class="btn" data-cat="${esc(c)}">${esc(c)} (${cnt[c]})</button>`; });
    filterEl.innerHTML = html;
    filterEl.querySelectorAll('button').forEach((b) =>
      b.addEventListener('click', () => render(b.getAttribute('data-cat'))));
  }
